...
//        )
//
////        let wLogger = ConsoleLogger(loggingLevel: .error)
...
//        await try? signClient.respond(topic: request.topic, response: .response(response))
//
//        pendingRequestsUpdatedRelay.accept(())
//    }
//
//    public func reject(request: WalletConnectSign.Request) {
//        await try? signClient.respond(topic: request.topic, response: .error(JSONRPCErrorResponse(id: request.id, error: JSONRPCErrorResponse.Error(code: 0, message: "reject by User"))))
//
//        pendingRequestsUpdatedRelay.accept(())
//    }
//
